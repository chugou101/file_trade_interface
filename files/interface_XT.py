...
class PBInterface(ABCInterface):

    Fields = {
        "order": ["BuySell", "Channel", "Price", "IssueCode","Quantity"],
    ...
    def _gen_order_from_report(self, ref_df):
        ...
        for _, row in ref_df.iterrows():
            ...
            result.append(Order(status=self._get_order_status(rec["OrderStatus"]),
                                ...
                                origin_status=rec["OrderStatus"], ref="PB", raw=rec))
        return result

    def _dump(self, template, instruction):
        orderid=self.gen_order_id()
        instruction.to_csv(self.csv_files[template]+str(orderid), encoding="utf-8",
    ...
    def order(self, security, amount, style=None, side='long', **kwargs):
        ...
        rec = {"BuySell": ["24", "23"][amount > 0] if not kwargs.get("BuySell", False) else kwargs["BuySell"],
    ...
    def cancel_order(self, orderid):
        ...
        order_id = self.gen_order_id()
        PB系统的撤单实现方式是终端显示指令编号，撤单时写入”cancel “+指令编号
        """
        rec = {"BuySell": "cancel",
               "Channel": orderid,
               }
    ...
    def position(self):
        ...
        for _, row in df.iterrows():
            ...
            if rec["PosID"]==1:  #空头持仓
                shortpositions.append(Position(security=securitycode,price=rec["LastPrice"],avg_cost=rec["AvgCost"],total_amount=rec["Quantity"],closeable_amount=rec["AvlQuantity"],value=rec["AssetValue"],side="short"))
                shortname.append(securitycode)
            elif rec["PosID"]==3:
                longpositions.append(Position(security=securitycode,price=rec["LastPrice"],avg_cost=rec["AvgCost"],total_amount=rec["Quantity"],closeable_amount=rec["AvlQuantity"],value=rec["AssetValue"],side="long"))
...
# ABCInterface.register(PBInterface)

if __name__ == "__main__":
    t = PBInterface(order_list_file="./OrderList.txt",
